...
BEGIN
  ...
  IF NEW.status = 'approved' THEN
    ...
    IF NOT student_exists THEN
      RAISE EXCEPTION 'Student not found, not a student, or already affiliated';
    END IF;
    ...
    BEGIN
      -- Update the student's affiliated instructor with explicit locking
      UPDATE profiles
      SET 
      ...
      IF affected_rows IS NULL OR affected_rows = 0 THEN
        RAISE EXCEPTION 'Failed to update student profile - student may already be affiliated';
      END IF;
    ...
    EXCEPTION
      WHEN lock_not_available THEN
        RAISE EXCEPTION 'Failed to process affiliation request: Profile is currently being modified';
      WHEN OTHERS THEN
